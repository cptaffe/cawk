...
/*
 * Rules:
 *   BEGIN block   — run before stream processing
 *   END block     — run after stream processing
 *   REGEX block   — x-expression: anchored match at current stream position;
 *                   $0 = full match (may span lines), $1/$2/... = capture groups.
 *                   Stream advances by len($0). First matching rule wins.
 *   block         — implicit /[^\n]*\n/ scanner (AWK-compat):
 *                   $0 = current line WITHOUT trailing newline.
 *
 * Expression patterns and range patterns are not supported.
 * Expression conditions go inside blocks as `if ($0 ~ /re/)` guards.
 */
rule:
    REGEX block
...
stmt:
  ...
  /* Inner x-expression: /re/ { } inside a block iterates over matches of re in $0.
   * Shift-reduce conflict with REGEX as expr is resolved by yacc default (shift),
   * which is correct: REGEX followed by '{' → RegexStmt, not expr. */
  | REGEX block
    { $$ = &RegexStmt{Regex: $1, Action: $2} }
  /* y/re/ { } — gap iteration (extension from sam) */
  | Y REGEX block
    { $$ = &YStmt{Regex: $2, Body: $3} }
  ;
...
if_stmt:
  ...
  | IF '(' expr ')' opt_nl simple_stmt
    { $$ = &IfStmt{Cond: $3, Then: []Stmt{$6}} }
  ;
...
expr:
  ...
  | expr '=' expr              { $$ = &AssignExpr{Op: "=",  Left: $1, Right: $3} }
